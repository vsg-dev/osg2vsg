...
std::string osg2vsg::createFragmentSourcePSC(const uint32_t& shaderModeMask, const uint32_t& geometryAttrbutes)
{
    std::string defines = createPSCDefinesString(shaderModeMask, geometryAttrbutes);
...
bool ShaderCompiler::compile(Shaders& shaders)
{
    ...
    for(auto& vsg_shader : shaders)
    {
        ...
        {
            DEBUG_OUTPUT << "Shader compilation failed:" << std::endl << vsg_shader->source().c_str() << std::endl;
